...
class WhiteLabelMachineNameModelVoidjob extends ObjectModel
{
     ...
     * @param int $spaceId
     * @param int $voidId
     * @return WhiteLabelMachineNameModelVoidjob
     */
    public static function loadByVoidId($spaceId, $voidId)
    {
        $voidJobs = new PrestaShopCollection('WhiteLabelMachineNameModelVoidjob');
        $voidJobs->where('space_id', '=', $spaceId);
        $voidJobs->where('void_id', '=', $voidId);
        $result = $voidJobs->getFirst();
        if ($result === false) {
            $result = new WhiteLabelMachineNameModelVoidjob();
        }
        return $result;
    }
    ...
    public static function isVoidRunningForTransaction($spaceId, $transactionId)
    {
        $result = DB::getInstance()->getValue(
            'SELECT id_void_job FROM ' . _DB_PREFIX_ . 'wlm_void_job WHERE space_id = "' . (int) $spaceId .
    ...
    public static function loadNotSentJobIds()
    {
        ...
        $result = DB::getInstance()->query(
            'SELECT id_void_job FROM ' . _DB_PREFIX_ . 'wlm_void_job WHERE state = "' . pSQL(self::STATE_CREATED) .
