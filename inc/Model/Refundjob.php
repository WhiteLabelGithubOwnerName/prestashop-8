...
class WhiteLabelMachineNameModelRefundjob extends ObjectModel
{
    ...
    {
        $this->refund_parameters = WhiteLabelMachineNameTools::base64Encode(serialize($params));
    }

    public function getRefundParameters()
    {
        $decoded = WhiteLabelMachineNameTools::base64Decode($this->refund_parameters, true);
        if ($decoded === false) {
    ...
    {
        $decoded = WhiteLabelMachineNameTools::base64Decode($this->failure_reason, true);
        if ($decoded === false) {
    ...
    {
        $this->failure_reason = WhiteLabelMachineNameTools::base64Encode(serialize($failureReason));
    }
     ...
     * @param int $spaceId
     * @param int $refundId
     * @return WhiteLabelMachineNameModelRefundjob
     */
    public static function loadByRefundId($spaceId, $refundId)
    {
        $refundJobs = new PrestaShopCollection('WhiteLabelMachineNameModelRefundjob');
        $refundJobs->where('space_id', '=', $spaceId);
        $refundJobs->where('refund_id', '=', $refundId);
        $result = $refundJobs->getFirst();
        if ($result === false) {
            $result = new WhiteLabelMachineNameModelRefundjob();
        }
        return $result;
    }
    ...
    public static function isRefundRunningForTransaction($spaceId, $transactionId)
    {
        $result = DB::getInstance()->getValue(
            'SELECT id_refund_job FROM ' . _DB_PREFIX_ . 'wlm_refund_job WHERE space_id = "' . (int) $spaceId .
    ...
    public static function loadNotSentJobIds()
    {
        ...
        $result = DB::getInstance()->query(
            'SELECT id_refund_job FROM ' . _DB_PREFIX_ . 'wlm_refund_job WHERE state = "' . pSQL(self::STATE_CREATED) .
    ...
    public static function loadNotAppliedJobIds()
    {
        $time = new DateTime();
        $result = DB::getInstance()->query(
            'SELECT id_refund_job FROM ' . _DB_PREFIX_ . 'wlm_refund_job WHERE state = "' . pSQL(self::STATE_APPLY) .
            '" AND date_upd < "' . pSQL($time->format('Y-m-d H:i:s')) . '"',
            false
        );
        $ids = array();
