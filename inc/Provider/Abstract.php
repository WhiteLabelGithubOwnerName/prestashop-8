 ...
 */
abstract class WhiteLabelMachineNameProviderAbstract
{
    ...
    private function loadData()
    {
        ...
        if ($cachedData !== null) {
            $decoded = WhiteLabelMachineNameTools::base64Decode($cachedData, true);
            if ($decoded === false) {
                $decoded = $cachedData;
            }
        ...
        try {
            ...
            Cache::store($this->cacheKey, WhiteLabelMachineNameTools::base64Encode(serialize($this->data)));
        } catch (\WhiteLabelMachineName\Sdk\ApiException $e) {
        } catch (\WhiteLabelMachineName\Sdk\Http\ConnectionException $e) {
        }
    }
}
