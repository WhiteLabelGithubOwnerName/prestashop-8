 ...
 */
class WhiteLabelMachineNameServiceLineitem extends WhiteLabelMachineNameServiceAbstract
{
    ...
    {
        $currencyCode = WhiteLabelMachineNameHelper::convertCurrencyIdToCode($cart->id_currency);
        $items = array();
        ...
        foreach ($summary['products'] as $productItem) {
            $item = new \WhiteLabelMachineName\Sdk\Model\LineItemCreate();
            $totalAmount = $this->roundAmount((float) $productItem['total_wt'], $currencyCode);
            $item->setAmountIncludingTax($totalAmount);
            $name = empty($productItem['name']) ? WhiteLabelMachineNameHelper::getModuleInstance()->l(
                'Product',
                'lineitem'
            ...
            foreach (array_keys($psTaxes) as $id) {
                $psTax = new Tax($id);
                $tax = new \WhiteLabelMachineName\Sdk\Model\TaxCreate();
            ...
            $item->setTaxes($taxes);
            $item->setType(\WhiteLabelMachineName\Sdk\Model\LineItemType::PRODUCT);
            if ($productItem['id_product'] == Configuration::get(WhiteLabelMachineNameBasemodule::CK_FEE_ITEM) ||
                $productItem['id_product'] == Configuration::get(WhiteLabelMachineNameBasemodule::CK_SURCHARGE_ITEM)) {
                $item->setType(\WhiteLabelMachineName\Sdk\Model\LineItemType::FEE);
        ...
        if ($shippingCosts > 0) {
            ...
            $item->setQuantity(1);
            $item->setShippingRequired(false);
            $item->setSku(WhiteLabelMachineNameHelper::getModuleInstance()->l('Shipping', 'lineitem'));
            $name = "";
            ...
            $name = empty($name) ? WhiteLabelMachineNameHelper::getModuleInstance()->l('Shipping', 'lineitem') : $name;
            $item->setName($name);
            if (! $taxCalculatorFound) {
                $taxRate = 0;
                $taxName = WhiteLabelMachineNameHelper::getModuleInstance()->l('Tax', 'lineitem');
            ...
            $item->setType(\WhiteLabelMachineName\Sdk\Model\LineItemType::SHIPPING);
        ...
        if ($wrappingCosts > 0) {
            ...
            $item->setName(WhiteLabelMachineNameHelper::getModuleInstance()->l('Wrapping Fee', 'lineitem'));
            $item->setQuantity(1);
            $item->setShippingRequired(false);
        ...
        if (count($summary['discounts']) > 0) {
            ...
            foreach ($summary['discounts'] as $discount) {
                $nameBase = empty($discount['description']) ? WhiteLabelMachineNameHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
        ...
        if ($discountOnly && $shippingItem != null) {
            $itemFreeShipping = new \WhiteLabelMachineName\Sdk\Model\LineItemCreate();
            $name = empty($discountOnly['name']) ? WhiteLabelMachineNameHelper::getModuleInstance()->l(
                'Shipping Discount',
                'lineitem'
            ) : $discountOnly['name'];
            ...
            $itemFreeShipping->setType(\WhiteLabelMachineName\Sdk\Model\LineItemType::DISCOUNT);
            $itemFreeShipping->setUniqueId('cart-' . $cart->id . '-discount-' . $discountOnly['id_cart_rule']);
            $items[] = $this->cleanLineItem($itemFreeShipping);
        }

        $cleaned = WhiteLabelMachineNameHelper::cleanupLineItems(
            $items,
            $orderTotal,
            $currencyCode
        );
    ...
    private function extractTaxName($psTax, $cart)
    {
        ...
        if (empty($translated) || strlen($translated) < 2) {
            ...
            return WhiteLabelMachineNameHelper::getModuleInstance()->l('Tax', 'lineitem');
        } else {
            return $translated;
        }
    }
    ...
    public function getItemsFromOrders(array $orders)
    {
        ...
        $cleaned = WhiteLabelMachineNameHelper::cleanupLineItems(
            $items,
            $orderTotal,
            WhiteLabelMachineNameHelper::convertCurrencyIdToCode($order->id_currency)
        );
        return $cleaned;
    }
    ...
    protected function getItemsFromOrdersInner(array $orders, $orderTotal)
    {
        ...
        foreach ($orders as $order) {
            /*@var Order $order */
            $currencyCode = WhiteLabelMachineNameHelper::convertCurrencyIdToCode($order->id_currency);
            ...
            foreach ($order->getProducts() as $orderItem) {
                ...
                $name = empty($orderItem['product_name']) ? WhiteLabelMachineNameHelper::getModuleInstance()->l(
                    'Product',
                    'lineitem'
                ...
                if ($orderItem['product_id'] == Configuration::get(WhiteLabelMachineNameBasemodule::CK_FEE_ITEM) ||
                    $orderItem['product_id'] == Configuration::get(WhiteLabelMachineNameBasemodule::CK_SURCHARGE_ITEM)) {
                    $item->setType(\WhiteLabelMachineName\Sdk\Model\LineItemType::FEE);
                    $item->setShippingRequired(false);
                }
            ...
            foreach ($order->getCartRules() as $orderCartRule) {
                ...
                $nameBase = empty($orderCartRule['name']) ? WhiteLabelMachineNameHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
                ) : $orderCartRule['name'];
                $discountItems = $this->getDiscountItems(
        ...
        if ($cartRule->gift_product != 0) {
            foreach ($existingLineItems as $exisitingLineItem) {
                if ($exisitingLineItem->getUniqueId() ==
                    ...
                    $item->setType(\WhiteLabelMachineName\Sdk\Model\LineItemType::DISCOUNT);
        ...
        if ($reductionPercent > 0) {
            if ($reductionProduct > 0) {
                ...
                if ($reductionProduct == - 2) {
                    $selectedProducts = WhiteLabelMachineNameCartruleaccessor::checkProductRestrictionsStatic(
                        $cartRule,
                        new Cart($cartIdUsed)
                    );
     ...
     *
     * @param \WhiteLabelMachineName\Sdk\Model\LineItemCreate $lineItem
     * @return \WhiteLabelMachineName\Sdk\Model\LineItemCreate
     */
    protected function cleanLineItem(\WhiteLabelMachineName\Sdk\Model\LineItemCreate $lineItem)
    {
