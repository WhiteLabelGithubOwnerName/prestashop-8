...
class WhiteLabelMachineNameModelMethodconfiguration extends ObjectModel
{
    ...
    public $fee_base = WhiteLabelMachineNameBasemodule::TOTAL_MODE_BOTH_INC;
    ...
    public static $definition = array(
        'table' => 'wlm_method_configuration',
    ...
    {
        $this->title = WhiteLabelMachineNameTools::base64Encode(serialize($title));
    }

    public function getTitle()
    {
        $decoded = WhiteLabelMachineNameTools::base64Decode($this->title, true);
        if ($decoded === false) {
            $decoded = $this->title;
        }
    ...
    {
        $this->description = WhiteLabelMachineNameTools::base64Encode(serialize($description));
    }

    public function getDescription()
    {
        $decoded = WhiteLabelMachineNameTools::base64Decode($this->description, true);
        if ($decoded === false) {
     ...
     * @param int $id
     * @param int $shopId
     * @return WhiteLabelMachineNameModelMethodconfiguration | false
     */
    public static function loadByIdWithChecks($id, $shopId)
    {
        ...
        $collection = new PrestaShopCollection('WhiteLabelMachineNameModelMethodconfiguration');
    ...
    public static function loadByConfigurationAndShop($spaceId, $configurationId, $shopId)
    {
        ...
        if ($result === false) {
            $result = new WhiteLabelMachineNameModelMethodconfiguration();
        }
        return $result;
    }
