...
class RefundRecoveryBankTransactionService {
	 ...
	 * @throws \WhiteLabelMachineName\Sdk\VersioningException
	 * @throws \WhiteLabelMachineName\Sdk\Http\ConnectionException
	 * @return \WhiteLabelMachineName\Sdk\Model\RefundRecoveryBankTransaction
	 */
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\WhiteLabelMachineName\Sdk\Model\RefundRecoveryBankTransaction',
				'/refund-recovery-bank-transaction/read'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\WhiteLabelMachineName\Sdk\Model\RefundRecoveryBankTransaction', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \WhiteLabelMachineName\Sdk\Model\EntityQuery $query The query restricts the refund recovery bank transactions which are returned by the search. (required)
	 * @throws \WhiteLabelMachineName\Sdk\ApiException
	 * @throws \WhiteLabelMachineName\Sdk\VersioningException
	 * @throws \WhiteLabelMachineName\Sdk\Http\ConnectionException
	 * @return \WhiteLabelMachineName\Sdk\Model\RefundRecoveryBankTransaction[]
