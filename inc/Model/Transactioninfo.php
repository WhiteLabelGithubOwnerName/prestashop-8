...
class WhiteLabelMachineNameModelTransactioninfo extends ObjectModel
{
    ...
    {
        $decoded = WhiteLabelMachineNameTools::base64Decode($this->labels, true);
        if ($decoded === false) {
            $decoded = $this->labels;
        }
    ...
    {
        $this->labels = WhiteLabelMachineNameTools::base64Encode(serialize($labels));
    }
    ...
    {
        $transactionInfos = new PrestaShopCollection('WhiteLabelMachineNameModelTransactioninfo');
        $transactionInfos->where('order_id', '=', $orderId);
        $result = $transactionInfos->getFirst();
        if ($result === false) {
            $result = new WhiteLabelMachineNameModelTransactioninfo();
        }
        return $result;
    }
