...
class WhiteLabelMachineNameModelTokeninfo extends ObjectModel
{
     ...
     * @param int $spaceId
     * @param int $tokenId
     * @return WhiteLabelMachineNameModelTokeninfo
     */
    public static function loadByToken($spaceId, $tokenId)
    {
        $tokenInfos = new PrestaShopCollection('WhiteLabelMachineNameModelTokeninfo');
        $tokenInfos->where('space_id', '=', $spaceId);
        $tokenInfos->where('token_id', '=', $tokenId);
        $result = $tokenInfos->getFirst();
        if ($result === false) {
            $result = new WhiteLabelMachineNameModelTokeninfo();
        }
        return $result;
    }
}
